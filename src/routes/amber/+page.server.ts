...
export const actions: Actions = {
    activate: async ({ request, locals: { supabase } }) => {
        const { data: { user }, error: authError } = await supabase.auth.getUser();
    ...
    complete: async ({ request, locals: { supabase } }) => {
        const { data: { user }, error: authError } = await supabase.auth.getUser();
        if (authError || !user) return { success: false, error: "Unauthorized" };
    ...
    acceptJointPlan: async ({ request, locals: { supabase } }) => {
        const { data: { user }, error: authError } = await supabase.auth.getUser();
        if (authError || !user) return { success: false, error: "Unauthorized" };
    ...
    declineJointPlan: async ({ request, locals: { supabase } }) => {
        const { data: { user }, error: authError } = await supabase.auth.getUser();
        if (authError || !user) return { success: false, error: "Unauthorized" };
    ...
    activateJointPlan: async ({ request, locals: { supabase } }) => {
        const { data: { user }, error: authError } = await supabase.auth.getUser();
        if (authError || !user) return { success: false, error: "Unauthorized" };
    ...
    cancelJointPlan: async ({ request, locals: { supabase } }) => {
        const { data: { user }, error: authError } = await supabase.auth.getUser();
        if (authError || !user) return { success: false, error: "Unauthorized" };
    ...
    shiftSingleTask: async ({ request, locals: { supabase } }) => {
        const { data: { user }, error: authError } = await supabase.auth.getUser();
        if (authError || !user) return { success: false, error: "Unauthorized" };
    ...
    updateIntensity: async ({ request, locals: { supabase } }) => {
        const { data: { user }, error: authError } = await supabase.auth.getUser();
        if (authError || !user) return { success: false, error: "Unauthorized" };
    ...
    scaleDurations: async ({ request, locals: { supabase } }) => {
        const { data: { user }, error: authError } = await supabase.auth.getUser();
        if (authError || !user) return { success: false, error: "Unauthorized" };
    ...
    shiftStartTimes: async ({ request, locals: { supabase } }) => {
        const { data: { user }, error: authError } = await supabase.auth.getUser();
        if (authError || !user) return { success: false, error: "Unauthorized" };
    ...
    markFailed: async ({ request, locals: { supabase } }) => {
        const { data: { user }, error: authError } = await supabase.auth.getUser();
        if (authError || !user) return { success: false, error: "Unauthorized" };
    ...
    extendSession: async ({ request, locals: { supabase } }) => {
        const { data: { user }, error: authError } = await supabase.auth.getUser();
        if (authError || !user) return { success: false, error: "Unauthorized" };
