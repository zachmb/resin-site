<script lang="ts">
    ...
    function selectStage(index: number) {
        ...
        requestAnimationFrame(() => document.getElementById(`workflow-tab-${activeStage}`)?.focus());
    }
...
<main class="landing-shell">
    <section class="hero">
        <div class="hero-copy">
            ...
            <div class="hero-actions">
                ...
                <a class="button secondary" href="https://testflight.apple.com/join/yV53qa1z" target="_blank" rel="noopener noreferrer"><Apple size={17} /> Join the iPhone beta</a>
            </div>
            <p class="hero-note"><Check size={14} /> Unlimited local planning on iPhone <span></span> No card required</p>
        </div>

        <div class="product-frame" role="img" aria-label="Resin turns a captured thought into a scheduled next step and a protected iPhone focus session">
            ...
            <div class="product-body">
                ...
                <div class="product-content">
                    <div class="product-heading"><div><small>MONDAY, AUGUST 10</small><h2>Make room for what matters.</h2></div><span class="preview-button">New thought</span></div>
                    ...
                    <article class="active-plan">
                        ...
                        <span class="preview-button focus-button"><LockKeyhole size={14} /> Begin focus</span>
                    </article>
            ...
            <div class="phone-preview">
                ...
                <span class="phone-button">Pause focus</span>
            </div>
        </div>
        ...
        <div class="workflow-shell">
            <div class="workflow-tabs" role="tablist" aria-label="How Resin works">
                {#each stages as stage, index}
                    ...
                    ><span>0{index + 1}</span>{stage.label}</button>
                {/each}
            </div>
            <div class="workflow-detail" id="workflow-panel" role="tabpanel" aria-labelledby={`workflow-tab-${activeStage}`} tabindex="0">
                ...
                <div class="workflow-demo">
                    ...
                    {:else if activeStage === 1}
                        <div class="steps-demo"><header><span>Launch the new page</span><small>52 min total</small></header><p><b><Check size={12} /></b><span>Review final copy</span><small>12m</small></p><p><b>2</b><span>Polish mobile sections</span><small>25m</small></p><p><b>3</b><span>Run final checks</span><small>15m</small></p><span class="place-button">Place this in my day <ArrowRight size={14} /></span></div>
                    {:else}
    ...
    <section class="focus-section">
        ...
        <div class="focus-preview">
            ...
            <div class="blocked-card"><span>reddit.com</span><ShieldCheck size={23} class="blocked-icon" /><h4>Not right now.</h4><p>You chose to focus on “Polish the mobile story.”</p><span class="return-button">Return to Resin</span></div>
        </div>
    </section>
    ...
    <section class="faq-section">
        ...
        <div class="faq-list">
            {#each faqs as faq, index}
                <div class="faq-item">
                    <button onclick={() => openFaq = openFaq === index ? null : index} aria-expanded={openFaq === index} aria-controls={`faq-answer-${index}`}><span>{faq[0]}</span><ChevronDown size={18} class={openFaq === index ? 'faq-icon open' : 'faq-icon'} /></button>
                    {#if openFaq === index}<p id={`faq-answer-${index}`}>{faq[1]}</p>{/if}
                </div>
            {/each}
        </div>
    </section>
    ...
    <section class="final-cta">
        ...
        <div class="final-actions"><a class="button amber" href="/login?mode=start&next=/">Make my first plan <ArrowRight size={17} /></a><a class="button dark-quiet" href="https://testflight.apple.com/join/yV53qa1z" target="_blank" rel="noopener noreferrer"><Apple size={17} /> Join the iPhone beta</a></div>
        <small>Unlimited local planning on iPhone · No card required · AI optional</small>
    </section>
</main>
...
<style>
    ...
    .product-body { height:calc(100% - 48px); display:grid; grid-template-columns:170px 1fr; }.product-body aside { padding:28px 20px 20px; border-right:1px solid rgba(39,35,31,.08); display:flex; flex-direction:column; background:#faf7f1; }.mini-logo { width:34px; height:34px; border-radius:10px; display:grid; place-items:center; color:#ebb16f; background:var(--ink); font:18px Georgia,serif; }.preview-nav { display:grid; gap:8px; margin-top:42px; }.preview-nav span,.preview-nav b { padding:10px 12px; border-radius:7px; color:#8d847a; font-size:10px; }.preview-nav b { color:#3f5948; background:#e5ece6; }.product-body aside > small { margin-top:auto; color:#aaa198; font-size:7px; }
    .product-content { padding:54px 56px; }.product-heading { display:flex; align-items:flex-end; justify-content:space-between; }.product-heading small,.active-plan small { color:#9b9187; font-size:8px; font-weight:800; letter-spacing:.12em; }.product-heading h2 { margin-top:9px; font-size:37px; letter-spacing:-.035em; }.preview-button { min-height:34px; padding:0 13px; border:1px solid rgba(39,35,31,.11); border-radius:8px; display:inline-flex; align-items:center; color:#fff; background:var(--ink); font-size:9px; font-weight:700; }
    ...
    .active-plan { min-height:128px; margin-top:13px; padding:18px; border:1px solid rgba(39,35,31,.1); border-radius:11px; display:grid; grid-template-columns:4px 1fr 54px auto; align-items:center; gap:16px; background:#fff; box-shadow:0 12px 28px rgba(39,35,31,.06); }.plan-mark { width:4px; height:76px; border-radius:4px; background:var(--amber); }.plan-main h3 { margin-top:7px; font-size:21px; }.plan-main p { margin-top:5px; color:#8d847a; font-size:9px; }.plan-duration { text-align:center; }.plan-duration strong,.plan-duration span { display:block; }.plan-duration strong { font:25px Georgia,serif; }.plan-duration span { color:#999087; font-size:7px; }.focus-button { padding:0 11px; border:0; gap:6px; background:var(--green); font-size:8px; font-weight:750; }
    ...
    .steps-demo { overflow:hidden; }.steps-demo header { padding:20px; display:flex; justify-content:space-between; border-bottom:1px solid rgba(39,35,31,.08); }.steps-demo header span { font:19px Georgia,serif; }.steps-demo header small,.steps-demo p small { color:#998f85; font-size:8px; }.steps-demo p { min-height:55px; padding:0 20px; border-bottom:1px solid rgba(39,35,31,.07); display:grid; grid-template-columns:25px 1fr auto; align-items:center; gap:9px; font-size:10px; }.steps-demo p b { width:20px; height:20px; border-radius:50%; display:grid; place-items:center; color:#fff; background:var(--green); font-size:8px; }.place-button { width:max-content; min-height:38px; margin:16px 20px; padding:0 13px; border-radius:8px; display:flex; align-items:center; gap:8px; color:#fff; background:var(--ink); font-size:8px; font-weight:750; }
    ...
    .blocked-card { width:295px; padding:25px; border:1px solid rgba(39,35,31,.12); border-radius:12px; position:absolute; right:0; bottom:0; color:var(--ink); background:#f3eee6; box-shadow:0 24px 55px rgba(0,0,0,.25); }.blocked-card > span:first-child { color:#91887e; font-size:8px; }:global(.blocked-icon) { display:block; margin-top:28px; color:var(--amber); }.blocked-card h4 { margin-top:12px; font-size:26px; }.blocked-card p { margin-top:7px; color:#7d746a; font-size:9px; line-height:1.6; }.return-button { width:100%; min-height:38px; margin-top:21px; border-radius:8px; display:flex; align-items:center; justify-content:center; color:#fff; background:var(--green); font-size:8px; font-weight:750; }
    ...
    @media(max-width:700px) {
        ...
        .product-frame { height:500px; margin:58px 0 0; border-radius:13px; }.product-toolbar { height:38px; }.product-body { height:calc(100% - 38px); grid-template-columns:58px 1fr; }.product-body aside { padding:15px 7px; }.mini-logo { width:28px; height:28px; margin:auto; }.preview-nav { margin-top:25px; }.preview-nav span,.preview-nav b { padding:7px 3px; text-align:center; font-size:6px; }.product-body aside > small { display:none; }.product-content { padding:30px 14px; }.product-heading h2 { font-size:21px; }.product-heading > .preview-button { display:none; }.capture-field { min-height:50px; margin-top:18px; padding:0 10px; font-size:7px; }.capture-field kbd { display:none; }.today-label { margin-top:25px; }.active-plan { min-height:112px; padding:12px 10px; grid-template-columns:3px 1fr 34px; gap:8px; }.plan-mark { height:67px; }.plan-main h3 { font-size:14px; }.plan-main p { font-size:6px; line-height:1.4; }.plan-duration strong { font-size:18px; }.active-plan > .focus-button { display:none; }.day-row { grid-template-columns:30px 1fr; }.phone-preview { width:145px; min-height:280px; right:-10px; bottom:-20px; padding:9px; border-width:5px; border-radius:25px; }.phone-preview > small { margin-top:16px; }.timer { width:82px; height:82px; margin:12px auto; }.timer strong { font-size:19px; }.phone-preview h3 { font-size:11px; }.shield-row { margin-top:10px; font-size:5px; }.phone-button { min-height:29px; margin-top:10px; }.platform-row { margin-top:45px; align-items:flex-start; flex-direction:column; gap:16px; }.platform-row > div { flex-wrap:wrap; gap:14px; }
